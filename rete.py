...
with tab_rete:
  ...
  if st.button("💾 Salva Modifiche Blocco IP", type="primary"):
...
with tab_hardware:
  ...
  with st.expander("🛠️ Aggiungi dettagli tecnici avanzati (Manuale)", expanded=True):
    ...
    if ip_disponibili_mostrati:
      scelta_mostrata = st.selectbox("1. Seleziona IP Dispositivo", ip_disponibili_mostrati, key=f"selettore_ip_{idx_selezionato}")
      ...
      with st.form(key=f"form_hw_{idx_selezionato}"):
        ...
        with col_f2:
          ...
          hw_tipologia = st.selectbox("Tipologia", ["PC / Macchina", "Stampante", "Switch"], index=0 if tipologia_esistente not in ["PC / Macchina", "Stampante", "Switch"] else ["PC / Macchina", "Stampante", "Switch"].index(tipologia_esistente))
        ...
        with col2:
          hw_ram = st.text_input("RAM", value=pulisci_valore(dettagli_esistenti.get("RAM", "16 GB")))
          hw_tipo_hd = st.text_input("Tipo HD", value=pulisci_valore(dettagli_esistenti.get("Tipo HD", "SSD")))
          hw_cap_hd = st.text_input("Capienza HD", value=pulisci_valore(dettagli_esistenti.get("Capienza HD", "")))
  ...
  df_inventario_modificato = st.data_editor(
      ...
      column_config={
          "Indirizzo IP": st.column_config.TextColumn("Indirizzo IP", disabled=True),
          "Nome Dispositivo": st.column_config.TextColumn("Nome Dispositivo"),
          "Tipologia": st.column_config.SelectboxColumn("Tipologia", options=["PC / Macchina", "Stampante", "Switch"], required=False),
          "Stato": st.column_config.SelectboxColumn("Stato", options=["🟢 Libero", "🔴 Occupato"], required=True),
      },
  ...
  st.markdown("---")
  st.markdown(f"##### 📥 Esporta Inventario Occupati - {sede_scelta['nome']}")
