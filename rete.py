...
def processa_stringa_hd(testo_capienza, tipo_hd_attuale):
  ...
  # Cerca se nel testo della capienza è esplicitamente indicato un tipo
  match_tipo = re.search(r'\b(SSD|HDD|NVMe)\b', cap_str, re.IGNORECASE)
  if match_tipo:
    trovato = match_tipo.group(1).upper()
    # Assegna il tipo trovato solo se il tipo attuale è vuoto o non specificato
    if not tipo_str or tipo_str == "-":
      tipo_str = trovato
    # Pulisce la stringa della capienza rimuovendo la parola del tipo trovata
    cap_str = re.sub(r'\b(SSD|HDD|NVMe)\b', '', cap_str, flags=re.IGNORECASE)
    cap_str = re.sub(r'[,;\s]+', ' ', cap_str).strip().strip(',').strip('-').strip()
...
idx_selezionato = st.selectbox(
    ...
    format_func=lambda i: f"{sedi_config[i]['nome']} — Rete: {sedi_config[i]['blocco']} / {sedi_config[i]['subnet']}",
)
...
with tab_rete:
  ...
  if st.button("💾 Salva Modifiche Rete", key=f"btn_salva_rete_{idx_selezionato}"):
...
with tab_hardware:
  ...
  with st.expander("🛠️ Aggiungi dettagli tecnici avanzati (Manuale)", expanded=False):
    ...
    if ip_disponibili_mostrati:
      scelta_mostrata = st.selectbox("Seleziona IP Dispositivo", ip_disponibili_mostrati, key=f"selettore_ip_{idx_selezionato}")
      ...
      with st.form(key=f"form_hw_{idx_selezionato}"):
        ...
        with col2:
          ...
          tipo_hd_esistente = pulisci_valore(dettagli_esistenti.get("Tipo HD", ""))
          opzioni_hd = ["", "SSD", "HDD", "NVMe"]
          idx_hd = opzioni_hd.index(tipo_hd_esistente) if tipo_hd_esistente in opzioni_hd else 0
        ...
        if btn_salva:
          ...
          dettagli_nuovi = {
              "Marca": hw_marca, "Modello": hw_modello, "S.O.": hw_so,
              "Processore": hw_cpu, "RAM": f"{hw_ram} GB",
              "Tipo HD": tipo_hd_fin, "Capienza HD": cap_hd_fin, "Garanzia": hw_garanzia,
          }
          ...
          if not idx_r.empty:
            ...
            df_rete_sede.loc[idx_r, "Stato"] = "🔴 Occupato" if pulisci_valore(hw_nome_macchina) else "🟢 Libero"
  ...
  df_inventario_modificato = st.data_editor(
      ...
      column_config={
          ...
          "Tipo HD": st.column_config.SelectboxColumn("Tipo HD", options=["", "SSD", "HDD", "NVMe"], required=False),
  ...
  if st.button("💾 Salva Modifiche Inventario", key=f"btn_salva_inv_{idx_selezionato}"):
